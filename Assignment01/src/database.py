...
class DatabaseManager:
    ...
    Database manager class to log tasks
    """
    ...
    def log_success(self, item_id, description, output_format, model, drive_url):
        ...
        try:
            ...
            self.logger.info(f"Successfully logged success for item {item_id}")
    ...
    def log_failure(self, item_id, description, output_format, model, error_message):
        ...
        try:
            ...
            self.logger.info(f"Successfully logged failure for item {item_id}")
    ...
    def get_success_failure_count_by_date_range(self, start_date, end_date):
        ...
        Returns:
            list: Danh sách tuple (date_str, success_count, failure_count)
                 List of tuples (date_str, success_count, failure_count)
        """
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            self.logger.error(f"Error getting statistics by date range: {e}")
            return []
